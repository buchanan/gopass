...
func (s *Action) stoken(ctx context.Context, name, pin, devid, seedpw string, clip, unsafe, pw, recurse bool) error {
	...
	if unsafe {
		out.Print(ctx, "%s", sec.Password())
		return nil
	}
	if pin == "" {
		if configPin, found := sec.Get("Pin"); found {
			pin = configPin
		}
	}
	if devid == "" {
		if configDEVID, found := sec.Get("DeviceID"); found {
			devid = configDEVID
		}
	}
	if seedpw == "" {
		if configSeedPW, found := sec.Get("SeedPassword"); found {
			seedpw = configSeedPW
		}
	}
	...
	if pw {
		...
		out.Warning(ctx, "%s lasts %ds \t|%s%s|", token, secondsLeft, strings.Repeat("-", int(interval-secondsLeft)), strings.Repeat("=", int(secondsLeft)))
	}
